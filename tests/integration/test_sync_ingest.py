...
async def _seed_tenant(
    ...
    *,
    ...
    """Seed tenant + actor + api token; return (tenant_id, actor_id, raw_token)."""
    ...
    try:
        async with factory() as session, session.begin():
            ...
            await session.execute(
                ...
                {"aid": actor_id, "tid": tenant_id, "dn": f"a-{slug}", "now": _NOW},
            )
            await session.execute(
                text(
...
async def p3_client(pg_container: str, app_settings: Settings):
    """Create an ASGI test client with a seeded admin tenant."""
    tenant_id, actor_id, raw = await _seed_tenant(
        pg_container,
...
@pytest.mark.asyncio
async def test_five_connectors_sync_under_60s(pg_container: str, respx_cassette) -> None:
...
@pytest.mark.asyncio
async def test_webhook_creates_delivery_record(pg_container: str, p3_client) -> None:
...
@pytest.mark.asyncio
async def test_webhook_duplicate_is_noop(pg_container: str, p3_client) -> None:
...
async def test_authoritative_wins_conflict(pg_container: str) -> None:
    ...
    """
    tenant_id, actor_id, _ = await _seed_tenant(
        pg_container,
...
async def test_sync_run_error_populates_error_summary(pg_container: str) -> None:
    ...
    try:
        ...
        assert run.status in ("partial", "failed"), f"expected partial/failed, got {run.status!r}"
        assert run.error_summary is not None, "error_summary must be populated on failure"
        assert "will-fail.openapi.yaml" in run.error_summary or "simulated" in run.error_summary
    finally:
        await engine.dispose()
...
@pytest.mark.asyncio
async def test_admin_sync_source_crud(p3_client) -> None:
    """POST / GET / PATCH / DELETE lifecycle for /v1/admin/sync-sources."""
    client: httpx.AsyncClient = p3_client["client"]
    token: str = p3_client["token"]
    ...
    # GET detail.
    r_get = await client.get(f"/v1/admin/sync-sources/{source_id}", headers=auth)
    ...
    # DELETE (soft).
    r_delete = await client.delete(f"/v1/admin/sync-sources/{source_id}", headers=auth)
    assert r_delete.status_code == 204

    # After soft-delete, GET returns 404.
    r_gone = await client.get(f"/v1/admin/sync-sources/{source_id}", headers=auth)
...
@pytest.mark.asyncio
async def test_admin_sync_run_list_and_superseded(pg_container: str, p3_client) -> None:
    ...
    # Detail.
    r_detail = await client.get(f"/v1/admin/sync-runs/{sync_run_id}", headers=auth)
    assert r_detail.status_code == 200
    assert r_detail.json()["status"] == "done"

    # Superseded facts (empty for this run — none written via conflict policy).
    r_sup = await client.get(f"/v1/admin/sync-runs/{sync_run_id}/superseded", headers=auth)
...
async def test_tenant_isolation_sync_source(pg_container: str) -> None:
    ...
    tenant_a_id, actor_a_id, token_a = await _seed_tenant(
        pg_container,
...
async def test_tenant_isolation_sync_run(pg_container: str) -> None:
    ...
    tenant_a_id, actor_a_id, token_a = await _seed_tenant(
        pg_container,
        slug=f"isob-a-{uuid.uuid4().hex[:8]}",
        roles=["admin"],
    )
    _, _, token_b = await _seed_tenant(
        pg_container,
        slug=f"isob-b-{uuid.uuid4().hex[:8]}",
        roles=["admin"],
    )
